...
> **目标**：界面**尽可能**使用系统组件与系统渲染，让新系统（macOS 26+）上自动呈现
> Liquid Glass（液态玻璃）外观，在旧系统上自动回退到经典样式，功能与代码都不受影响。
> "系统组件优先"是**默认原则而非绝对禁令**：系统组件确实覆盖不了的需求，
> 允许少量自定义绘制（约束见第 1 条）。

## 硬性规则

1. **控件优先用 SwiftUI 系统组件**：`Button` / `Toggle` / `TextField` / `Form` /
   `NavigationSplitView` / `ToolbarItem` 等。
   **默认原则**：能用系统组件实现的功能一律用系统组件，不自行绘制。
   **例外（允许少量自定义绘制）**：系统组件确实无法满足需求时——
   如特殊交互、行为不符、性能要求——允许自定义 `NSView` 子类、手绘圆角矩形、
   自绘按钮贴图、`Canvas` 绘制交互控件等，但必须满足：
   - 视觉与系统风格一致（对齐系统语义色、圆角、间距、材质）；
   - 保留基本可达性（键盘操作、VoiceOver 标签），或提供等价系统替代；
   - 代码注释说明"为什么系统组件不可行"，防止后续误用。
